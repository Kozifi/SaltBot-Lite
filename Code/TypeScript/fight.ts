...
module.exports.run = async (bot, message, args) => {
    ...
    if (getID(args[0]) == message.author.id && !args[1]) message.channel.send('Hmmm, alright then.');

    //Check if the input link is an image
    if (isImageUrl(user1) && isImageUrl(user2)) {
        let status = await message.channel.send('Starting fight...');
...
async function StartBattle(message, user1name, user2name, user1, user2, bot) {
    ...
    var attacks = [ //attacks[title, status, effect]
    ...
    let sentmsg = await message.channel.send(fightembed);
    ...
    for (var i = 0; i < 20; i++) {
        setTimeout(function () {
            if (Math.random() >= 0.3) //attack
            {
                ...
                {
                    turn = false;
                    let a: any[] = attacks[rdn(0, attacks.length - 1)]; //decides attack
                    let dmg: number = rdn(a[2], a[3]); //decides damage
            ...
            else //buff
            {
                if (turn) {
                    turn = false;
                    let a: any[] = self[rdn(0, self.length - 1)]; //decides attack
                    let dmg: number = rdn(a[2], a[3]); //decides damage
                    fightembed.addField(`**${user1name}** ${a[0].replace("@", `**${user2name}**`)}`, a[1].replace("@", dmg.toString()) + " 🍉") //Adds fight message - X DOES Y, Y AFFECT
                    ...
                    turn = true;
                    let a: any[] = self[rdn(0, self.length - 1)];
                    let dmg: number = rdn(a[2], a[3]);
                    fightembed.addField(`**${user2name}** ${a[0].replace("@", `**${user1name}**`)}`, a[1].replace("@", dmg.toString()) + " 🍍")
                    user2HP = user2HP + dmg;
                    if (user2HP < 0) user2HP = 0;
                    fightembed.setDescription(`🍉 ${user1name}: ${user1HP.toString()}\n🍍 ${user2name}: ${user2HP.toString()} HP\n${blank}`)
            ...
            if (user1HP < 1 || user2HP < 1) {
                ...
                if (user1HP == 0) w = [user2name, "🍍", user2HP.toString(), user2];
